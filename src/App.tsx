...
function ServiceIcon({ name }: { name: ServiceIconName }) {
  if (name === 'network') return <svg viewBox="0 0 120 80" aria-hidden="true"><path d="M18 57 48 31l30 26M48 31h43M48 31V12" /><rect x="7" y="50" width="22" height="17" rx="3" /><rect x="80" y="22" width="28" height="19" rx="3" /><circle cx="48" cy="12" r="7" /><circle cx="78" cy="57" r="8" /></svg>
  ...
  if (name === 'support') return <svg viewBox="0 0 120 80" aria-hidden="true"><path d="M29 42a31 31 0 0 1 62 0" /><path d="M29 42v13c0 5 4 9 9 9h4V44h-4c-5 0-9 4-9 9ZM91 42v13c0 5-4 9-9 9h-4V44h4c5 0 9 4 9 9Z" /><path d="M78 65c0 7-7 10-16 10h-7" /><circle cx="51" cy="75" r="3" /></svg>
...
function App() {
  ...
  useLayoutEffect(() => {
    ...
    return () => { cleanupHeroPointer(); ctx.revert() }
  }, [])
  ...
  return <main className="site premium-site" id="top" ref={siteRef}>
    ...
    <section className="services-showcase" id="services"><div className="section-intro premium-intro"><p className="eyebrow">HOW WE HELP</p><h2>Technology that pulls its weight.</h2><p>Four connected services. One team that takes ownership when something matters.</p></div><div className="service-grid">{services.map((service) => <article className="service-card" key={service.title}><div className="service-card__top"><span>{service.number}</span><ServiceIcon name={service.icon} /></div><div><h3>{service.title}</h3><p className="service-card__outcome">{service.outcome}</p><p>{service.detail}</p></div><button type="button" onClick={openChat}>Ask about this service <span>&rarr;</span></button></article>)}</div></section>
